...
export default function FileUploader() {
    ...
    const removeUploadedFile = async (file: UploadedFile) => {
        try {
            ...
            toast.className = "fixed top-4 right-4 bg-red-600 text-white px-4 py-2 rounded-lg z-50 shadow-lg animate-in slide-in-from-right duration-300";
    ...
    const getPreviewUrl = (file: UploadedFile) => {
        ...
        const isPreviewable = file.isImage || file.isVideo || file.type.startsWith("audio/") || file.name.match(/\.(mp3|wav|ogg|aac|flac|m4a)$/i);

        if (isPreviewable) {
            // Encode the file name to handle special characters and spaces
            const encodedFileName = encodeURIComponent(file.fileName);
            return `${Config.CdnUrl ? Config.CdnUrl : window.location.origin}/file/${encodedFileName}`;
        }
        return null;
    };
    ...
    const copyToClipboard = async (text: string) => {
        try {
            ...
            toast.className = "fixed top-4 right-4 bg-green-600 text-white px-4 py-2 rounded-lg z-50 shadow-lg animate-in slide-in-from-right duration-300";
            document.body.appendChild(toast);
            setTimeout(() => {
                toast.className += " animate-out slide-out-to-right duration-300";
                setTimeout(() => document.body.removeChild(toast), 300);
    ...
    const isPreviewable = (file: UploadedFile) => {
        return file.isImage || file.isVideo || file.type.startsWith("audio/") || file.name.match(/\.(mp3|wav|ogg|aac|flac|m4a)$/i);
    };
                ...
                <Card className="border-0 bg-transparent">
                    <CardContent className="p-0">
                        <div className="space-y-4">
                            ...
                            <p className="text-gray-400 text-sm">Drag and drop file atau klik untuk memilih file (Max. 50MB per file)</p>

                            {/* Upload Area */}
                            <div
                                className={`relative overflow-hidden rounded-lg border-2 border-dashed cursor-pointer transition-all duration-300 ${
                                    isDragOver ? "border-blue-400 bg-blue-500/5 scale-[1.02]" : "border-gray-600 hover:border-gray-500 hover:bg-gray-800/20"
                                }`}
                                onDrop={handleDrop}
                        ...
                        <CardContent className="p-6">
                            ...
                            <div className="space-y-4">
                                {uploadingFiles.map(file => (
                                    <div key={file.id} className="flex items-center gap-4 p-4 bg-gray-800/50 rounded-lg animate-in slide-in-from-left duration-300">
                                        <div className="flex-shrink-0">
                                            ...
                                            {file.status === "success" && <CheckCircle className="w-8 h-8 text-green-500 animate-in zoom-in duration-300" />}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {file.status === "success" && <p className="text-green-400 text-sm animate-pulse">Upload completed!</p>}

                                            {file.status === "error" && <p className="text-red-400 text-sm">{file.error}</p>}
                                        </div>

                                        <Button variant="ghost" size="sm" onClick={() => removeUploadingFile(file.id)} className="text-gray-400 hover:text-white transition-colors">
                                            <X className="w-4 h-4" />
                                        </Button>
                                    </div>
                                ))}
                        ...
                        <CardContent className="p-6">
                            ...
                            <div className="grid gap-4">
                                {uploadedFiles.map((file, index) => (
                                    <div key={file.id} className="flex items-center gap-4 p-4 bg-gray-800/50 rounded-lg hover:bg-gray-800/70 transition-all duration-200 animate-in slide-in-from-bottom" style={{ animationDelay: `${index * 100}ms` }}>
                                        {getFileIcon(file)}
                                        ...
                                        <div className="flex items-center gap-2">
                                            {isPreviewable(file) && (
                                                <>
                                                    <Button variant="ghost" size="sm" onClick={() => window.open(getPreviewUrl(file)!, "_blank")} className="text-purple-400 hover:text-purple-300 transition-colors">
                                                        Preview
                                                    </Button>
                                                    <Button variant="ghost" size="sm" onClick={() => copyToClipboard(getPreviewUrl(file)!)} className="text-green-400 hover:text-green-300 transition-colors" title="Copy preview URL">
                                                        <Copy className="w-4 h-4" />
                                                    </Button>
                                                </>
                                            )}
                                            <Button variant="ghost" size="sm" onClick={() => window.open(getDirectFileUrl(file), "_blank")} className="text-blue-400 hover:text-blue-300 transition-colors" title="Download file">
                                                <Download className="w-4 h-4" />
                                            </Button>
                                            <Button variant="ghost" size="sm" onClick={() => removeUploadedFile(file)} className="text-gray-400 hover:text-red-400 transition-colors" title="Delete file">
                                                <X className="w-4 h-4" />
                                            </Button>
                                        </div>
                                    </div>
